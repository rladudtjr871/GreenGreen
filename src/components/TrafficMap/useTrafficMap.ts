...
import type { MapBounds, MapCoordinate, MapViewport } from "@/types/map";
...
export function useTrafficMap() {
  ...
  if (isIntersectionQueryEnabled && intersectionsQuery.isLoading) {
    guide = {
      eyebrow: "교차로 확인 중",
  ...
  const serviceBadgeText = !isIntersectionQueryEnabled
    ? `ZOOM! ZOOM!`
